...
async def geetest_handle(user_id: str, uid: str, ssbq_data: dict):
    ...
    #header = mihoyo_headers(cookie=cookie_info.cookie, q=f'role_id={uid}&server={server_id}',)
    ...
    token, err_string = await geetest_handle_init(api_key=captcha_api_key, gt=gt, challenge=challenge, page_url=page_url)
    if err_string != "":
        logger.warning('原神实时便签', '➤', f'用户试图绕过验证码失败：打码平台返回错误{err_string}')
        return 1, f'{uid}查询实时便签失败，米哈游服务器返回1034疑似验证码，绕过验证码失败：{err_string}'
